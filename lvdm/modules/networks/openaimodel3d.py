...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, batch_size=None, attn_mask=None):
        for layer in self:
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, **kwargs):
        ...
        if x.shape[0]==2: #为了加attn_mask
            attn_mask = x[1].unsqueeze(0)
            attn_mask = rearrange(attn_mask, 'b c t h w -> (b t) c h w')
            x = x[0].unsqueeze(0)
        else: 
            attn_mask = None
        ...
        context = context.repeat_interleave(repeats=t, dim=0) #context是文本tokenembedding
        ...
        for id, module in enumerate(self.input_blocks):
            if(id<6): #只在下层用attn_mask
                h = module(h, emb, context=context, batch_size=b, attn_mask=attn_mask)
            else:
        ...
        # h = self.middle_block(h, emb, context=context, batch_size=b, attn_mask=attn_mask)
        h = self.middle_block(h, emb, context=context, batch_size=b)

        # for module in self.output_blocks:
        #     h = torch.cat([h, hs.pop()], dim=1) #这一步应该是unet中的跳连 把encoder的输出和decoder的输入拼接起来
        #     h = module(h, emb, context=context, batch_size=b, attn_mask=attn_mask)
        
        for id, module in enumerate(self.output_blocks):
            h = torch.cat([h, hs.pop()], dim=1) #这一步应该是unet中的跳连 把encoder的输出和decoder的输入拼接起来
            if id>5: #只在下层用attn_mask
                h = module(h, emb, context=context, batch_size=b, attn_mask=attn_mask)
            else:
                h = module(h, emb, context=context, batch_size=b)
